...
from QMIX import agent


def init_agent(env, args):
    return agent.TeamAgent(env, args)
...
def save_replays(policy_store_dir, save_dir, curriculum_file, task_to_assign=None, tmp_seed=35, tmp_ckpt=1000):
    ...
    args.player_spawn_teammate_distance = 1
    args.player_team_size = 8
    ...
    args.member_num = 8
    args.lr = 1e-5
    team_agent = init_agent(nmmo_env, args)

    team_agent.mixnet.load_state_dict(torch.load(f'./QMIX/xfy/team_{tmp_ckpt}.pth'))
    team_agent.target_mixnet.load_state_dict(torch.load(f'./QMIX/xfy/team_{tmp_ckpt}.pth'))
    for i in range(8):
        team_agent.agents[i].net.load_state_dict(torch.load(f'./QMIX/xfy/agent_{tmp_ckpt}.pth'))
    ...
    while True:
        with torch.no_grad():
            ...
            all_teams_load_QMIX = True
            if all_teams_load_QMIX:
                for i in range(args.member_num * 8):
                    new_action = torch.zeros_like(actions[0]).to(evaluator.device)
                    raw_action = team_agent.agents[i % 8].take_action(torch.Tensor(o).to(evaluator.device)[i])
                    new_action[0] = raw_action[0]
                    new_action[1] = raw_action[1]
                    new_action[8] = raw_action[2]
                    new_action[11] = raw_action[3]
                    # if i == 0:
                    #     print(raw_action)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--buffer_capacity', type=int,
                        default=200)  # How many trajectories are kept in the buffer. Out of this capacity, the trajs are first-in-first-out (FIFO), leaving only the most recent ones.
    ...
    if getattr(eval_args, "replay_mode", False):
        for tmp_seed in [35, 36, 37]:
            for tmp_ckpt in [1000]:
                logging.info("Generating replays from the checkpoints in %s", eval_args.policy_store_dir)
                print(f"tmp_seed={tmp_seed}, tmp_ckpt={tmp_ckpt}")
                save_replays(eval_args.policy_store_dir, eval_args.replay_save_dir,
                            eval_args.task_file, eval_args.task_index, tmp_seed=tmp_seed, tmp_ckpt=tmp_ckpt)
    else:
